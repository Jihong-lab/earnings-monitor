...
$slugs = @('googl','aapl','msft','samsung','alibaba','icbc','hsbc','cba','mufg','hdfc-bank')
$results = @()
foreach ($slug in $slugs) {
  $t0 = Get-Date
  Write-Output "===== $slug ====="
  try {
    $resp = Invoke-RestMethod -Uri "http://localhost:3000/api/cron/refresh?only=$slug" -Headers @{ Authorization = "Bearer $secret" } -TimeoutSec 600
...
$results | ConvertTo-Json | Set-Content -Path backfill-missing-result.json -Encoding utf8
